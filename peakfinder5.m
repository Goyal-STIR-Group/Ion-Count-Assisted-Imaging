function varargout = peakfinder5(x0, sel, thresh, extrema, includeEndpoints, interpolate,minPeakWidth,minProminence,minPeakDistance)
...
% Modified by Xinglin He, last updated January 19, 2024
...
if nargin < 7 || isempty(minPeakWidth)
    minPeakWidth = 0;
end

if nargin < 8 || isempty(minProminence)
    minProminence = 0;
end

if nargin < 9 || isempty(minPeakDistance)
    minPeakDistance = 0;
end
...
%Change zero derivative values to slightly negative to prevent consecutive
%zero derivatives
ind = find(dx0(1:end-1).*dx0(2:end) < 0)+1; % Find where the derivative changes sign (compare each element with the one that comes after it)

% Include endpoints in potential peaks and valleys as desired
if includeEndpoints
    x = [x0(1);x0(ind);x0(end)]; %include the first and last points with the points where the sign changed
    ind = [1;ind;len0];
    minMag = min(x);
    leftMin = minMag;
else
...
if len >= 2 % Function with peaks and valleys
    ...
    if includeEndpoints
        ...
        if signDx(1) <= 0 % The first point is larger or equal to the second
            if signDx(1) == signDx(2) % Want alternating signs (handle the case where the alternating signed pattern is distrupted by the added points)
                x(2) = [];
                ind(2) = [];
                len = len-1;
            end
    ...
    while ii < len
        ...
        if foundPeak
            tempMag = minMag; %Reseting the tempMag back to min
            foundPeak = false;
        end

        % Found new peak that was lager than temp mag and selectivity larger
        %   than the minimum to its left.
        if x(ii) > tempMag && x(ii) > sel && ind(ii) < ind(end) - 3
            if x0(ind(ii) + 1) >= x0(ind(ii) + 2) && x0(ind(ii) + 2) >= x0(ind(ii) + 3)
                tempLoc = ii;
                tempMag = x(ii); %storing the new peak
            end
        end
        ...
        if ~foundPeak && tempMag > sel 
            foundPeak = true; % We have found a peak
            peakProminence = tempMag - leftMin; % Calculate peak prominence
            leftMin = x(ii);%consider left min in the next loop but consider right min here
            if peakProminence >= minProminence % Check against the minimum prominence
                peakLoc(cInd) = tempLoc; % Add peak to index
                peakMag(cInd) = tempMag;
                cInd = cInd+1;
            end
        elseif x(ii) < leftMin % Set new left minima
            leftMin = x(ii);
        end
    end
    ...
    % Check end point
    if includeEndpoints
        if x(end) > tempMag && x(end) > leftMin + sel %Check if the end point satisfy the condition
            peakLoc(cInd) = len;
            peakMag(cInd) = x(end);
            cInd = cInd + 1;
    ...
    elseif ~foundPeak
        if x(end) > tempMag && x(end) > leftMin + sel %still check the endpoint
            peakLoc(cInd) = len;
            peakMag(cInd) = x(end);
            cInd = cInd + 1;
    ...
    if cInd > 1
        ...
        [peakMags,xInd] = max(x);
        if includeEndpoints && peakMags > sel
            peakInds = ind(xInd);
        else
            peakMags = [];
            peakInds = [];
...
if ~isempty(minPeakDistance) && length(peakInds) > 1 % Won't have effect unless there's more than one peak
    indDiff = diff(peakInds);
    minPeakDistance = round(minPeakDistance / 10e-9) + 1;
    validInds = [true; indDiff >= minPeakDistance]; % Downside:Fisrt peak is always consider true to the condition
    peakInds = peakInds(validInds);
    peakMags = peakMags(validInds);
end

peakWidths = zeros(size(peakInds));
for i = 1:length(peakInds)
    % Find left and right indices around the peak
    % Downside:Hard to deal with noisy dataset
    leftIdx = peakInds(i);
    while leftIdx > 1 && x0(leftIdx) >= x0(leftIdx - 1)
        leftIdx = leftIdx - 1;
        if leftIdx > 2 && x0(leftIdx) < x0(leftIdx - 1)
            if x0(leftIdx - 1) >= x0(leftIdx - 2)
                leftIdx = leftIdx - 1;
            end
        end
    end
    ...
    rightIdx = peakInds(i);
    while rightIdx < len0 && x0(rightIdx) >= x0(rightIdx + 1)
        rightIdx = rightIdx + 1;
        if rightIdx < len0 - 1 && x0(rightIdx) < x0(rightIdx + 1)
            if x0(rightIdx + 1) >= x0(rightIdx + 2)
                rightIdx = rightIdx + 1;
            end
        end
    end

    % Calculate peak width as the difference between right and left indices
    peakWidths(i) = rightIdx - leftIdx + 1;
end

% Apply minimum peak width filter
if ~isempty(minPeakWidth)
    minPeakWidth = round(minPeakWidth / 10e-9) + 1;
    validPeaks = peakWidths >= minPeakWidth;
    peakInds = peakInds(validPeaks);
    peakMags = peakMags(validPeaks);
end

if interpolate && ~isempty(peakMags)%Get a more precise locations of the peak(?)
